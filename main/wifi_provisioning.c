/* Módulo de Wi-Fi Provisioning 
   Proporciona funcionalidad para configurar WiFi mediante BLE o SoftAP
*/
                          ...
                          int32_t event_id, void* event_data)
{
    ...
    if (event_base == WIFI_PROV_EVENT) {
        ...
        // Convertir la IP a string para notificar a la aplicación principal
        char ip_str[16];
        snprintf(ip_str, sizeof(ip_str), IPSTR, IP2STR(&event->ip_info.ip));
        
        // Si hay un callback registrado, notificar la IP
        if (connected_callback != NULL) {
            connected_callback(ip_str);
        }
...
/* Implementation of functions from header file */
...
void wifi_provisioning_wait_for_connection(EventGroupHandle_t event_group)
{
    xEventGroupWaitBits(event_group, WIFI_CONNECTED_EVENT, true, true, portMAX_DELAY);
}
...
EventGroupHandle_t wifi_provisioning_init(void)
{
    ...
    if (!provisioned) {
...
#ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_BLE
        // Información de conexión BLE
        ESP_LOGI(TAG, "------------------------------------");
        ESP_LOGI(TAG, "BLE Provisioning Information");
        ESP_LOGI(TAG, "------------------------------------");
        ...
        ESP_LOGI(TAG, "------------------------------------");
        ESP_LOGI(TAG, "SoftAP Provisioning Information");
        ESP_LOGI(TAG, "------------------------------------");
        ESP_LOGI(TAG, "SSID: %s", service_name);
        ESP_LOGI(TAG, "Password: %s", service_key ? service_key : "");
        ESP_LOGI(TAG, "------------------------------------");
#endif /* CONFIG_EXAMPLE_PROV_TRANSPORT_BLE */
    } else {
